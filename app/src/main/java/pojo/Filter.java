...
public class Filter {
    ...
    public static boolean fromMainToStorage(Filter[][] main_matrix, int from_index, int main_height, Filter[][] storage_matrix, int to_index){
        ...
        for (int x = 0; x < main_width; x++){
            main_matrix[x][main_height - 2].makeSpace();
        }
        return true;
    }
    ...
    public static boolean stringToMatrix(String string, Filter[][] matrix, int height){
        ...
        for (int y = 0; y < height; y++){
            for (int x = 0; x < width; x++){
                try {
                    matrix[x][y].setColorAndShape(Integer.parseInt(string.substring(y * width * 2 + x * 2, y * width * 2 + x * 2 + 1)), Integer.parseInt(string.substring(y * width * 2 + x * 2 + 1, y * width * 2 + x * 2 + 2)));
